...
async def _build_wiki(home: Path, *, text: str, vector: list[float]) -> None:
    ...
    try:
        conn = db.conn
        await conn.execute(
            "INSERT INTO raw_sources (id, source_type, title, url, text, provenance)"
            " VALUES (1, 'dev_event', 'probe', '', :text, '{}')",
            {"text": text},
        )
        await conn.execute(
            "INSERT INTO chunks (id, owner_type, owner_id, seq, text)"
            " VALUES (1, 'raw_source', 1, 0, :text)",
            {"text": text},
        )
        await conn.execute(
...
Before writing, open `tests/test_config.py` and copy its existing minimal-payload helper if one exists rather than duplicating `_minimal_config` — DRY across the test suite matters here because a config field added later must break one helper, not two.
...
@pytest.mark.asyncio
async def test_sourced_is_frozen() -> None:
...
async def _read_peer(
    ...
    *,
    ...
    except (TimeoutError, Exception):  # noqa: B014,BLE001 -- a bad peer is never fatal
        return []
    finally:
        await db.close()
...
Run: `uv run pytest -q && uv run ruff check . && uv run ruff format --check . && uv run mypy wikiforge`
Expected: clean. If ruff rejects `except (TimeoutError, Exception)` as redundant, write `except Exception` alone with a comment noting that `TimeoutError` is a subclass of `Exception` in Python 3.13 and is therefore already covered.
...
**Note on `run_peers_list`:** it builds the embedding provider only to read `.model`, which for the local provider is a cheap attribute — it must not trigger a model download or load. If `build_embedding_provider` turns out to load weights eagerly, read `cfg.embedding.local_model` / `cfg.embedding.voyage_model` through the same `effective_*` helper instead and leave a comment saying why.
...
async def _wiki_with_event(home: Path, name: str, *, path: str, ts: str, summary: str) -> None:
    ...
    try:
        repo = Repository(db)
        await repo.ensure_dev_event_files()
        source = await repo.insert_raw_source(_event(0, ts=ts, summary=summary))
        await db.conn.execute(
            "INSERT INTO dev_event_files (source_id, path) VALUES (:sid, :p)",
            {"sid": source.id, "p": path},
        )
        await db.conn.commit()
    finally:
        await db.close()
...
Check `Repository.insert_raw_source`'s real name and signature before writing the fixture (`grep -n "async def insert_raw_source" -A 6 wikiforge/storage/repository.py`) and match it exactly; if it returns an id rather than a `RawSource`, adjust `_wiki_with_event` accordingly.
...
- Test: `tests/test_changelog_federated.py`

**Interfaces:**
- Consumes: Tasks 6 and 8.
- Produces: `ChangelogEntry` gains `origin: str = ""`; `Changelog` gains `by_origin: dict[str, int]`; `build_file_impact` gains `peer_events: list[Sourced[RawSource]] = []`.
...
from wikiforge.federation.fanout import Sourced
from wikiforge.models.domain import RawSource
from wikiforge.ops.changelog import Changelog, ChangelogEntry, format_changelog


def _event(summary: str) -> RawSource:
    return RawSource(
        id=1,
        source_type="dev_event",
        title=summary,
        url="",
        text=f"## Request (why)\n{summary}\n",
        provenance={"ts": "2026-07-21T10:00:00+00:00", "type": "feature", "summary": summary},
    )
...
def test_footer_reports_per_origin() -> None:
    ...
    assert "global" in out
    assert "1 from peer wiki" in out or "from global" in out
...
Read `wikiforge/ops/changelog.py` first and match `Changelog`'s real field names and order — the constructor calls above must compile against the actual dataclass, adding only `origin` and `by_origin`.

- [ ] **Step 2: Run test to verify it fails**

Run: `uv run pytest tests/test_changelog_federated.py -v`
Expected: FAIL — `TypeError: ChangelogEntry.__init__() got an unexpected keyword argument 'origin'`.

- [ ] **Step 3: Extend the dataclasses and the render**

Add `origin: str = ""` to `ChangelogEntry` and `by_origin: dict[str, int] = field(default_factory=dict)` to `Changelog`. In `format_changelog`, append `  [alias]` to an entry's line when `entry.origin` is non-empty, and extend the coverage footer with a per-origin breakdown, e.g.:

```python
    peer_counts = {k: v for k, v in log.by_origin.items() if k}
    if peer_counts:
        detail = ", ".join(f"{n} from {alias}" for alias, n in sorted(peer_counts.items()))
        lines.append(f"Of these, {detail} (peer wikis).")
```

- [ ] **Step 4: Federate the two services**

In `run_changelog`, after `build_changelog` produces the local log, fan out the same selection against peers with `read_only=True` and merge:

```python
    from wikiforge.federation.fanout import active_peers, fan_out

    peer_entries = await fan_out(
        active_peers(cfg),
...
Add `select_peer_events` to `ops/changelog.py`: the same selection `build_changelog` performs, called with `read_only=True`, returning `list[RawSource]` and never touching coverage counters (a peer cannot tell us which of *our* files have history — only which events exist). Merge the returned events into `log.entries` as `ChangelogEntry(event=…, matched_by="files", origin=alias)` and recompute `by_origin`.

In `run_impact`, do the same only for the **file** target: source and topic targets stay local, because a peer's citation and topic graph is its own and merging them needs cross-wiki topic identity — an explicit non-goal (spec §3). Add a one-line note to `build_file_impact`'s docstring saying so.

- [ ] **Step 5: Run tests to verify they pass**

Run: `uv run pytest tests/test_changelog_federated.py tests/test_changelog_render.py tests/test_changelog_build.py tests/test_impact_file.py tests/test_audit_impact.py -v`
Expected: all PASS.

- [ ] **Step 6: Commit**

```bash
git add wikiforge/ops/changelog.py wikiforge/ops/impact.py wikiforge/services.py tests/test_changelog_federated.py
...
Before finishing, confirm that the concrete providers record `purpose` verbatim into `llm_calls` (`grep -n "purpose" wikiforge/llm/anthropic_provider.py wikiforge/llm/claude_code_provider.py`). If a provider rewrites or drops the purpose, the tag must instead be applied where `CostTracker.record` is called — the ledger depends on the stored value, not the passed one.
...
async def _run_vectors(ctx: JobContext) -> str:
    ...
    stats = await flush_dev_events(
        ctx.repo, ctx.embedder(), None, ctx.cfg, digests=False, max_batches=0
    )
...
Two repository methods this needs may not exist yet — check and add them next to their neighbours if missing, each with a docstring:
...
(Query in `raw_sources.sql`: `-- name: count_dev_event_files^` / `SELECT COUNT(*) AS n FROM dev_event_files;`. If `dev_event_files` may be absent, catch `sqlite3.OperationalError` and return 0 — the local wiki always has it after `ensure`, but the method must not be a trap for a caller that skips the ensure.)

Also confirm `flush_dev_events`'s real signature and that `max_batches=0` means "embed only, no digests"; if the parameter semantics differ, call it the way `run_capture_flush` does for the vectors-only case.
